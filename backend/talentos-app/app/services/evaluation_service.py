...
async def _evaluate_descriptive(db: Session, question: Question, candidate_answer: str) -> Evaluation:
    rubrics_block = "\n".join(
        f'- "{m.rubric.name}" (weight {m.weight_percentage}%): {m.evaluation_criteria}'
        for m in question.rubric_maps
    )

    raw_output = await agent_client.invoke(
        "EVALUATION_DESCRIPTIVE_AGENT",
        {
    ...
    result = LLMEvaluation.model_validate(raw_output)
...
async def evaluate_answer(db: Session, question: Question, payload: EvaluationRequest) -> Evaluation:
    if question.question_type == "descriptive":
        ...
        evaluation = await _evaluate_descriptive(db, question, payload.candidate_answer)
...
async def evaluate_batch(
    db: Session, answers: list[EvaluationRequest]
    ...
    for answer in answers:
        ...
        evaluation = await evaluate_answer(db, question, answer)
