...
import { toneForScopeType } from "../lib/tone";
import { KNOWN_SERVICES, type OrgUser, type PrincipalType, type RoleAssignment, type RoleDefinition, type ScopeType, type ServiceName, type ServicePrincipal } from "../types";
...
export default function RoleAssignmentsPage() {
  ...
  function load() {
    if (!orgId) return;
    Promise.all([listRoleAssignments(orgId), listOrgUsers(orgId), listServicePrincipals(orgId), listRoleDefinitions(orgId)])
    ...
    <div className="page">
      ...
      <Card>
        {assignments === null ? (
          ...
          <EmptyState
            ...
            description="Assign a role to a user or service principal to grant access."
            action={canManage ? <Button onClick={openCreate}>New assignment</Button> : undefined}
          />
        ) : (
      ...
      {revoking && (
        <ConfirmDialog
          ...
          message={`Remove "${revoking.role_name}" from ${principalLabelFor(revoking.principal_type, revoking.principal_id, users, servicePrincipals)}?`}
          confirmLabel="Revoke"
